...
const Home = () => {
  return (
    <main className="overflow-hidden">
      ...
      {/* Stats Section */}
      <section className="py-24 sm:py-40 bg-secondary text-white relative overflow-hidden">
        {/* Refined Background Elements */}
        <div className="absolute inset-0 opacity-[0.02] pointer-events-none dot-matrix" />
        <div className="absolute top-0 left-1/4 w-px h-full bg-gradient-to-b from-transparent via-white/20 to-transparent opacity-10" />
        <div className="absolute top-0 left-2/4 w-px h-full bg-gradient-to-b from-transparent via-white/20 to-transparent opacity-10" />
        <div className="absolute top-0 left-3/4 w-px h-full bg-gradient-to-b from-transparent via-white/20 to-transparent opacity-10" />
        
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-16 lg:gap-y-0">
            {[
              { label: "Patients Treated", value: "1000+", icon: Users, suffix: "Lives Impacted" },
              { label: "Years Experience", value: "10+", icon: History, suffix: "Clinical Excellence" },
              { label: "Diabetes Cases", value: "500+", icon: Activity, suffix: "Specialized Care" },
              { label: "Patient Rating", value: "5.0", icon: Star, suffix: "Trust & Reliability" }
            ].map((stat, i) => (
              <motion.div
                key={i}
              ...
              >
                ...
                <div className="relative mb-8 sm:mb-12">
                  <div className="w-14 h-14 sm:w-24 sm:h-24 bg-white/5 rounded-[2rem] flex items-center justify-center text-accent/60 border border-white/5 group-hover:border-accent/30 group-hover:bg-accent/10 transition-all duration-700 relative z-10">
                    <stat.icon size={28} className="sm:w-12 sm:h-12 transition-transform duration-700 group-hover:scale-110" />
                  </div>
                ...
                {/* Value */}
                <div className="text-4xl sm:text-7xl lg:text-8xl font-display font-bold text-white mb-4 tracking-tighter tabular-nums">
                  {stat.value}
                </div>

                {/* Labels */}
                <div className="space-y-2">
                  <h4 className="text-[10px] sm:text-xs uppercase tracking-[0.4em] font-bold text-white/80">
                    {stat.label}
                  </h4>
                  <p className="text-[9px] uppercase tracking-[0.2em] font-medium text-white/20 italic font-serif">
                    {stat.suffix}
                  </p>
                </div>
              </motion.div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10">
          ...
          >
            ...
            <motion.div 
              ...
              className="absolute -bottom-10 -right-10 bg-secondary text-white p-10 rounded-[3rem] shadow-2xl hidden sm:block border border-white/10"
            >
              ...
              <div className="space-y-6 mb-16">
                {[
                  ...
                  <motion.div 
                    ...
                    className="flex items-start gap-8 p-8 rounded-[2.5rem] bg-medical-bg border border-gray-50 group hover:bg-white hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500"
                  >
                    <div className={`w-16 h-16 ${item.color} rounded-2xl flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}>
                      <item.icon size={32} />
                    </div>
                    <div>
                      <h4 className="text-xl font-bold text-secondary mb-2">{item.title}</h4>
                      <p className="text-secondary/50 leading-relaxed text-lg">{item.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
